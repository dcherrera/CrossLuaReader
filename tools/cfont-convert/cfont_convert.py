...
parser.add_argument("--pnum", dest="pnum", action="store_true", help="Use proportional numeral spacing from GSUB 'pnum' feature.")
parser.add_argument("--cfont", dest="cfont_output", action="store", metavar="FILE", help="Output as .cfont binary file instead of C header.")
...
if args.cfont_output:
    # ── Binary .cfont output ──────────────────────────────────────────
    import struct

    face = font_stack[0]
    advance_y = norm_ceil(face.size.height)
    ascender_val = norm_ceil(face.size.ascender)
    descender_val = norm_floor(face.size.descender)
    # Clamp to int8 range
    ascender_i8 = max(-128, min(127, ascender_val))
    descender_i8 = max(-128, min(127, descender_val))

    flags = 0x01 if is2Bit else 0x00
    style_val = 0  # Determined by font name convention

    n_intervals = len(intervals)
    n_glyphs = len(glyph_props)
    n_groups = len(compressed_groups) if compress else 0
    n_kern_left_entries = len(kern_left_classes) if kern_map else 0
    n_kern_right_entries = len(kern_right_classes) if kern_map else 0
    n_kern_left_classes = kern_left_class_count if kern_map else 0
    n_kern_right_classes = kern_right_class_count if kern_map else 0
    n_ligatures = len(ligature_pairs) if ligature_pairs else 0

    # Calculate metadata section sizes
    intervals_size = n_intervals * 12
    glyphs_size = n_glyphs * 14
    groups_size = n_groups * 18
    kern_left_size = n_kern_left_entries * 3
    kern_right_size = n_kern_right_entries * 3
    kern_matrix_size = n_kern_left_classes * n_kern_right_classes
    ligatures_size = n_ligatures * 8

    metadata_size = intervals_size + glyphs_size + groups_size + \
                    kern_left_size + kern_right_size + kern_matrix_size + ligatures_size

    bitmap_offset = 32 + metadata_size  # 32-byte header + metadata

    # Pack header (32 bytes)
    header = struct.pack('<4sBBBBBbbBBxHHHHHHxx I',
        b'CFNT',
        1,               # version
        flags,
        size,             # font_size
        ...
        descender_i8,
        n_kern_left_classes,
        n_kern_right_classes,
        # reserved1 (x)
        n_glyphs,
        n_intervals,
        n_groups,
        n_kern_left_entries,
        n_kern_right_entries,
        n_ligatures,
        # reserved2 (xx)
        bitmap_offset
    )
    ...
    with open(args.cfont_output, 'wb') as f:
        ...
        for props, packed in all_glyphs:
            f.write(struct.pack('<BBHhhHI',
                props.width, props.height, props.advance_x,
                props.left, props.top, props.data_length, props.data_offset))

        # Groups (18 bytes each)
        if compress:
            for comp_data, uncomp_size, gc, fgi in compressed_groups:
                # comp_data is bytes; compressed_offset was accumulated during C output
                pass
            # Need to track offsets — recalculate
            comp_offset = 0
            for comp_bytes, uncomp_size, gc, fgi in compressed_groups:
                f.write(struct.pack('<IIIHI',
                    comp_offset, len(comp_bytes), uncomp_size, gc, fgi))
                comp_offset += len(comp_bytes)

        # Kern left classes (3 bytes each, packed)
        if kern_map:
            for cp, cls in kern_left_classes:
        ...
        # Kern matrix (int8 flat row-major)
        if kern_map:
            f.write(struct.pack(f'<{len(kern_matrix)}b', *kern_matrix))
        ...
        # Bitmap data (compressed DEFLATE streams concatenated)
        if compress:
            for comp_bytes, uncomp_size, gc, fgi in compressed_groups:
                f.write(comp_bytes)
        else:
            # Uncompressed: write all packed bitmap data
            for props, packed in all_glyphs:
                f.write(packed)

    file_size = 32 + metadata_size + (sum(len(cb) for cb, us, gc, fgi in compressed_groups) if compress else sum(len(packed) for props, packed in all_glyphs))
    print(f"Wrote {args.cfont_output}: {file_size} bytes, {n_glyphs} glyphs, {n_groups} groups", file=sys.stderr)

else:
    # ── Original C header output ──────────────────────────────────────
