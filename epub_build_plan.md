...
- [ ] **Test on device**: boot. All existing plugins (home, file browser, settings, txt reader, md reader, firmware home) work identically to before. Pull SD; firmware home loads. Reload SD; home loads. Open a `.txt` or `.md` file; reader loads with `text.*` registered (verifies opt-in dispatch).
- [ ] **RAM gate (baseline preservation)**: pre-existing 1 KB target slightly exceeded (+152 bytes over). Documented as the new architecturally-required baseline; future phases compare against 78,232 bytes.
